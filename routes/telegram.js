...
const checkToken = function (req, res, next) {
  if (req.params.token !== process.env.TELEGRAM_BOT_TOKEN) {
    res.status(498).
      json({success: false, message: 'Failed token', token: req.params})
  }
  next()
}
...
const attachChatToCity = (city, chatId) => {
  ...
  if (cityId !== undefined) {
    TelegramBotChat.findOrCreate(
      {where: {chat_id: chatId}, defaults: {city_id: cityId}}).
      spread((chat, created) => {
        if (created === false) {
          chat.update({
            city_id: cityId,
...
const getCourses = (ctx) => {
  ...
  TelegramBotChat.findOne({where: {chat_id: ctx.chat.id}}).
    then(async (chat) => {
      ...
      let rates = ExchangeRate.findAll({
        where: {
          city_id: userCityId, hidden: 0, published: 1, date_update: {
            [Op.gte]: yesterday,
          },
        },
        attributes: [field, 'name', 'date_update', 'info', 'phones'],
      }).then(async rates => {
        let responseText = '<b>Выгодные курсы</b> обмена валюты, по запросу "<b>' +
          ctx.message.text + '</b>":\n\r'
        ...
        _.forEach(rates, (value, key) => {
          if (parseFloat(value[field]) === parseFloat(arrBest[field])) {
            ...
            responseCoursesText += '<b>' + _.unescape(value.name) + '</b>\n\r' +
              ...
              '<b>Время обновления: </b>' + day + '.' + month + '.' + year +
              ' ' +
              hours + ':' + minutes + '\n\r'
            if (value.phones) {
        ...
        return ctx.replyWithHTML(replyText,
          Markup.inlineKeyboard([
            Markup.urlButton('Более подробно на сайте',
              url),
          ]).extra())
      })
    })
